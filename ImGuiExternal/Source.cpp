...
void InputHandler() {
	std::cout << "[*] Inputhnadler" << std::endl;
	for (int i = 0; i < 5; i++) {
...
void Draw() {
	...
	char  ter[128],FpsInfo[128], Sizex[128], Sizey[128];
	sprintf(ter, "Fg", ImGui::GetFontSize());
	sprintf(FpsInfo, "FPS: %0.f", ImGui::GetIO().Framerate);
	sprintf(Sizex, "Size:%0.f", ImGui::GetIO().DisplaySize.x);
	sprintf(Sizey, "%0.f", ImGui::GetIO().DisplaySize.y);
	//ImGui::GetFontSize();
	auto pDrawList = ImGui::GetWindowDrawList();
	RGBA White = { 255,0,255,255 };
	DrawStrokeText(30, 20, &White, ter);
	DrawStrokeText(30, 44, &White, FpsInfo);
	DrawStrokeText(30, 60, &White, std::ctime(&end_time));
	DrawStrokeText(30,80, &White, Sizex);
	DrawStrokeText(100, 80, &White, Sizey);
}

void Render() {
	if (GetAsyncKeyState(VK_F9) & 1) ShowMenu = !ShowMenu;
	...
	if (DirectX9.pDevice->BeginScene() >= 0) {
		std::cout << "[*] DirectX9->BeginScene" << std::endl;
		ImGui::Render();
...
void MainLoop() {
	...
	while (DirectX9.Message.message != WM_QUIT) {
		...
		if (ForegroundWindow == Process.Hwnd) {
			std::cout << "[*] MainLoop Foreground Window" << std::endl;
...
bool DirectXInit() {
	...
	std::cout << "[*] Directx Init" << std::endl;
...
LRESULT CALLBACK WinProc(HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_SIZE:
		if (DirectX9.pDevice != NULL && wParam != SIZE_MINIMIZED) {
			std::cout << "[*] case Wm Size" << std::endl;
			ImGui_ImplDX9_InvalidateDeviceObjects();
...
*/
void getfn() {
	Process.ID = ForegroundWindowProcessID;
...
DWORD WINAPI ProcessCheck(LPVOID lpParameter) {
	std::cout << "[*] ProcessCheck" << std::endl;
	while (true) {
